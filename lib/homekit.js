...
function isNestThermostat(device)
{
  return 'nest_thermostat' === device.driver.id;
}
...
function createNewDevice(device, server, newPairedDevices, id)
{
  ...
  let Identify = HAS.predefined.Identify(1, undefined, function(value, callback)
    ...
    Name = HAS.predefined.Name(4, device.zone.name + ' ' + device.name),
  ...
  for (let key in device.capabilities)
    ...
    switch (device.capabilities[key].id)
    {
      ...
      case 'onoff':
        ...
        if(device.class == 'amplifier') break;
      ...
      case 'target_temperature':
        ...
        else
        {
          variableState = HAS.predefined.TargetHeatingCoolingState(characteristicNum++, undefined, (value, callback) =>
          {
            ...
            if (isNestThermostat(device)) callback(HAS.statusCodes.OK);
            else updateStatus (device, variable, key, callback);
          });
          if (!isNestThermostat(device)) updateStatusOnHKNoNull(targeTemperature2thermostatMode(sDV(device.state[key], 20)), variableState, 'TargetHeatingCoolingState', device);
          if (test) testStab(variableState, key, device);
          ...
          variableState = HAS.predefined.CurrentHeatingCoolingState(characteristicNum++);
          if (!isNestThermostat(device)) updateStatusOnHKNoNull(targeTemperature2currentThermostatMode(sDV(device.state[key], 20)), variableState, 'CurrentHeatingCoolingState', device);
          if (test) testStab(variableState, key, device);
      ...
      case 'windowcoverings_state':
        if ('dim' in device.capabilities == false)
        {
          ...
          updateStatusOnHKNoNull(stateCover2proc(sDV(device.state[key], 'idle'),opposite), variableState, key, device);
          if (test) testStab(variableState, key, device);
          ...
          updateStatusOnHKNoNull(stateCover2proc(sDV(device.state[key], 'idle'),opposite), variableState, key+'TargetPosition', device);
          if (test) testStab(variableState, key, device);
          ...
          updateStatusOnHKNoNull(stateCover2num(sDV(device.state[key], 'idle'),opposite), variableState, key+'PositionState', device);
          if (test) testStab(variableState, key, device);
      ...
      case 'dim':
        if (device.class == 'windowcoverings')
        {
          ...
          variableState = HAS.predefined.CurrentPosition(characteristicNum++);
          updateStatusOnHKNoNull( (opposite?-1:1) * sDV(device.state[key], 1) * 100 + (opposite?100:0), variableState, key+'dim', device);
          if (test) testStab(variableState, key, device);
          ...
          updateStatusOnHKNoNull( (opposite?-1:1) * sDV(device.state[key], 1) * 100 + (opposite?100:0), variableState, key, device);
          if (test) testStab(variableState, key, device);
          ...
          {
            variableState = HAS.predefined.PositionState(characteristicNum++);
            updateStatusOnHKNoNull(stateCover2num(sDV(device.state.windowcoverings_state , 'idle'),opposite), variableState, key + 'windowcoverings_state', device);
            if (test) testStab(variableState, key, device);
      ...
      case 'volume_set':
        ...
        if (device.class == 'amplifier')
        {
          ...
          updateStatusOnHKNoNull(sDV(device.state.onoff, false), variableState, 'onoff', device);
          if (test) testStab(variableState, 'onoff', device);

          capabilities.push(variableState);
          arrState['onoff'] = variableState;

          // Brightness aka Volume
          variableState = HAS.predefined.Brightness(characteristicNum++, undefined, (value, callback) =>
          {
      ...
      case 'measure_pm25':
        ...
        variableState = HAS.predefined.AirQuality(characteristicNum++);
        updateStatusOnHKNoNull(p252AirQuality(sDV(device.state[key], 0)), variableState, key, device);
        if (test) testStab(variableState, key, device);
            ...
            if (variable != null && variable != undefined)
            {
              switch (device.capabilities[key].id)
              {
                ...
                case 'onoff':
                  ...
                  if ('light_hue' in device.capabilities && variable == false)return;
                  break;
                ...
                case 'target_temperature':
                  ...
                  if (!isNestThermostat(device) && 'thermostat_mode' in device.capabilities == false && 'eurotronic_mode' in device.capabilities == false)
                  {
